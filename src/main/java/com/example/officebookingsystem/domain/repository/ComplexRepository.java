...
public interface ComplexRepository extends JpaRepository<Complex, Long> {
    ...
    // @Query(value =
    // "SELECT new
    // com.example.officebookingsystem.domain.dto.response.ComplexResponse(c.id,
    // c.complexName, c.address, s.name, d.name, count(b))" +
    // "FROM Complex c INNER JOIN City s on s.id = c.city.id " +
    // "INNER JOIN District d on d.id = c.district.id INNER JOIN Building b on
    // b.complex.id = c.id " +
